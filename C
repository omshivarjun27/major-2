...
from core.memory.cloud_sync import CloudSyncAdapter, CloudSyncConfig, SyncRecord, StubCloudBackend
...
@pytest.fixture
async def stub_backend() -> StubCloudBackend:
...
async def test_stub_upsert_stores_records(stub_backend: StubCloudBackend) -> None:
    ...
    assert count == 3
    assert len(stub_backend._records) == 3
...
async def test_stub_delete_removes_record(stub_backend: StubCloudBackend) -> None:
    ...
    await stub_backend.upsert([record])
    deleted = await stub_backend.delete(["delete_me"])

    assert deleted == 1
    assert "delete_me" not in stub_backend._records
...
async def test_adapter_periodic_flush(sync_adapter: CloudSyncAdapter) -> None:
    ...
    try:
        ...
        backend = sync_adapter._backend
        assert len(backend._records) == len(records)
    finally:
...
async def test_adapter_health_reports_queue_size(sync_adapter: CloudSyncAdapter) -> None:
    ...
    await sync_adapter.enqueue(records)
    health = sync_adapter.health()
    queue_size = health["pending"]

    assert queue_size == 3
    assert health["backend"]["records"] == 0
