...
class MemoryMaintenance:
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        # Task 3: Save index
        try:
            await asyncio.to_thread(self._indexer.save)
    ...
    async def enforce_retention(self) -> int:
        ...
        # Delete expired memories in a thread (sync I/O)
        ...
        deleted = await asyncio.to_thread(_delete_expired)
        if deleted > 0:
    ...
    async def compact_index(self):
        ...
        await asyncio.to_thread(self._indexer.compact)
    ...
    async def backup(self, backup_path: Path) -> bool:
        ...
        # Run all file I/O in a thread to avoid blocking the event loop
