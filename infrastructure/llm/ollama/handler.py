...
class OllamaHandler:
    ...
    def __init__(self):
        """Initialize the Ollama API handler with minimal setup."""
        try:
            config = get_config()
            self.api_key = config["OLLAMA_VL_API_KEY"]
            self.model_id = config["OLLAMA_VL_MODEL_ID"]
            self.max_tokens = config["MAX_TOKENS"]
            self.temperature = config["TEMPERATURE"]
            
            logger.info(f"Initializing Ollama handler with model: {self.model_id}")
            ...
            # Set up async HTTP client for Ollama API
            ...
            logger.error(f"Error during Ollama handler initialization: {e}")
            self.is_ready = False
            self._verified = False
    ...
    async def verify_connection(self):
        ...
        self.is_ready = True
        self._verified = True
        logger.info(f"Ollama handler verified with model {self.model_id}")
        return True
    ...
    async def model_choice_with_analysis(self, image, query: str):
        ...
        try:
            ...
            if model_choice not in ["OLLAMA", "QWEN"]:
                logger.warning(f"Invalid model choice: {model_choice}, defaulting to ollama:qwen3-vl:235b-instruct-cloud")
                model_choice = "ollama:qwen3-vl:235b-instruct-cloud"
            
            logger.info(f"Model choice: {model_choice}, analysis available: {bool(ollama_analysis)}")
